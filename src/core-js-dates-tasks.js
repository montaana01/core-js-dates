...
function getNextFriday(date) {
  ...
  let daysToFridayAdd = (5 - currentDay + 7) % 7;
  if (daysToFridayAdd === 0) daysToFridayAdd = 7;
  day.setDate(day.getDate() + daysToFridayAdd);
  return day;
}
...
function formatDate(date) {
  ...
  const minutes = dateToFormat.getUTCMinutes().toString().padStart(2, '0');
  const seconds = dateToFormat.getUTCSeconds().toString().padStart(2, '0');
  const halfOfDay = hours >= 12 ? 'PM' : 'AM';
  hours = hours % 12 || 12;

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${halfOfDay}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let day = 1; day <= date; day += 1) {
    const checkingDay = new Date(year, month - 1, day);
    if (checkingDay.getDay() === 0 || checkingDay.getDay() === 6) weekends += 1;
  }
  return weekends;
}
...
function getWeekNumberByDate(date) {
  const guessWeekDate = new Date(
    Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())
  );
  guessWeekDate.setUTCDate(
    guessWeekDate.getUTCDate() + 4 - (guessWeekDate.getUTCDay() || 7)
  );
  const startOfYear = new Date(Date.UTC(guessWeekDate.getUTCFullYear(), 0, 1));

  return Math.ceil(((guessWeekDate - startOfYear) / 86400000 + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  if (currentDate > date && currentDate.getDay() === 5) return currentDate;
